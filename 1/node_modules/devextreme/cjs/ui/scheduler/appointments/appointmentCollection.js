...
var SchedulerAppointments = function(_CollectionWidget) {
    ...
    _proto._renderAppointment = function(element, settings) {
        ...
        if (settings.virtual) {
            var deferredColor = this.invoke("getResourceManager").getAppointmentColor({
            ...
            if (this.isAgendaView) {
                ...
                config.createPlainResourceListAsync = function(rawAppointment) {
                    return resourceManager._createPlainResourcesByAppointmentAsync(rawAppointment)
                }
            }
    ...
    _proto._applyResourceDataAttr = function($appointment) {
        var resourceManager = this.invoke("getResourceManager");
        var resources = resourceManager.getResourcesFromItem(this._getItemData($appointment));
        if (resources) {
    ...
    _proto._sortAppointmentsByStartDate = function(appointments) {
        appointments.sort(function(a, b) {
            var result = 0;
            var firstDate = new Date(_expressionUtils.ExpressionUtils.getField(this.option("key"), "startDate", a.settings || a)).getTime();
            var secondDate = new Date(_expressionUtils.ExpressionUtils.getField(this.option("key"), "startDate", b.settings || b)).getTime();
