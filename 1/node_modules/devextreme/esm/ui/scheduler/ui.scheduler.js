...
} from "./appointments/settingsGenerator";
import {
    utils
} from "./utils";
import {
...
class Scheduler extends Widget {
    ...
    _appointmentsConfig() {
        var config = {
            ...
            onContentReady: () => {
                ...
                null === (_this$_workSpace = this._workSpace) || void 0 === _this$_workSpace ? void 0 : _this$_workSpace.option("allDayExpanded", this._isAllDayExpanded(filteredItems))
            }
        };
        return config
    }
    ...
    _excludeAppointmentFromSeries(rawAppointment, newRawAppointment, exceptionDate, isDeleted, isPopupEditing, dragEvent) {
        ...
        var newAppointment = createAppointmentAdapter(this.key, newRawAppointment);
    ...
    dayHasAppointment(day, rawAppointment, trimTime) {
        var getConvertedToTimeZone = date => getTimeZoneCalculator(this.key).createDate(date, {
            path: "toGrid"
        });
        var appointment = createAppointmentAdapter(this.key, rawAppointment);
    ...
    setTargetedAppointmentResources(rawAppointment, element, appointmentIndex) {
        ...
        if (null !== groups && void 0 !== groups && groups.length) {
            var resourcesSetter = getResourceManager(this.key)._dataAccessors.setter;
            var workSpace = this._workSpace;
            var getGroups;
    ...
    showAppointmentTooltip(appointment, element, targetedAppointment) {
        if (appointment) {
            var settings = utils.dataAccessors.getAppointmentSettings(element);
            var deferredColor = getResourceManager(this.key).getAppointmentColor({
